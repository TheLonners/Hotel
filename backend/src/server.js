...
app.put("/api/reservations/:id", asyncRoute(async (req, res) => {
...
app.post("/api/reservations/:id/rooms", asyncRoute(async (req, res) => {
  const reservation = getReservation(Number(req.params.id));
  if (!reservation) return res.status(404).json({ error: "Reserva no encontrada." });
...
app.delete("/api/reservations/:id/rooms/:roomId", (req, res) => {
...
app.post("/api/reservations/:id/payments", asyncRoute(async (req, res) => {
...
app.post("/api/reservations/:id/attachments", upload.single("file"), asyncRoute(async (req, res) => {
  const reservation = getReservation(Number(req.params.id));
  if (!reservation) return res.status(404).json({ error: "Reserva no encontrada." });
...
app.put("/api/cleaning/:roomId", (req, res) => {
...
app.put("/api/blocks/:id", asyncRoute(async (req, res) => {
...
app.post("/api/airbnb-sync/feeds", asyncRoute(async (req, res) => {
  const feed = createAirbnbFeed(req.body);
  res.status(201).json(feed);
}));

app.put("/api/airbnb-sync/feeds/:id", asyncRoute(async (req, res) => {
...
app.delete("/api/airbnb-sync/feeds/:id", (req, res) => {
  deleteAirbnbFeed(Number(req.params.id));
  res.json({ ok: true });
});

app.post("/api/airbnb-sync/feeds/:id/sync", asyncRoute(async (req, res) => {
...
app.post("/api/airbnb-sync/import-preview", memoryUpload.single("file"), asyncRoute(async (req, res) => {
  if (!req.file) return res.status(400).json({ error: "Sube un archivo CSV o Excel exportado de Airbnb." });
  res.json(previewAirbnbImport(req.file.buffer, req.file.originalname));
}));

app.post("/api/import/excel/preview", memoryUpload.single("file"), asyncRoute(async (req, res) => {
...
app.post("/api/import/excel/confirm", asyncRoute(async (req, res) => {
  const result = confirmImport(req.body.sessionId, { force: Boolean(req.body.force) });
  res.json(result);
}));

app.post("/api/import/rooms/preview", memoryUpload.single("file"), asyncRoute(async (req, res) => {
...
app.post("/api/billing-account/export.xlsx", (req, res) => {
  sendXlsx(res, "cuenta-cobro-vista-montana.xlsx", buildBillingWorkbook(req.body || {}));
});

app.post("/api/billing-account/export.pdf", (req, res) => {
